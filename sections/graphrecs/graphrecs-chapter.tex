...
This can be done either with information about the interaction between user and an item by adding side-information to the edge between the two nodes, such that the edge $(U, I)$ that previously indicated an interaction between an user and an item is extended with edge side-information such as $(U, r, I)$ where $r$ is a rating that user $U$ has assigned to item $I$\cite{aggarwal2016recommender}. 
...
Adding this side-information allows us to connect items through other nodes than just users and vice-versa.
