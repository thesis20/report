...
\textit{Neural Graph Collaborative Filtering} (NGCF), proposed by Wang et al. in \cite{NGCF}, is a method for collaborative filtering, employing a user-item bipartite graph structure.
Different to \textit{IS-UserBased} and \textit{CAMF} that make rating predictions, this method instead produces implicit ratings in the form of a top-$N$ list.
...
The embedding from user to user is found by multiplying the first weight matric at the given layer with the user embedding from the previous layer.
...
The loss function is defined in \autoref{eqn:ngcf-loss}.
