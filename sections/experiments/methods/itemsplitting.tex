...
Both of these methods results in a list of the top-$N$ similar users, from which it is possible to estimate the missing rating $r_{ij}$ of the current user $u$ for a given item $i$,as seen on \autoref{eql:israj}.
\begin{equation}
    \label{eqn:israj}
