...
The main difference is that RMSE places a larger penalization on large errors, such that given a test set with four items RMSE would prefer a system that makes an error of 2 on three ratings and 0 on the fourth, to one that makes an error of 3 on one rating and 0 on the others.
MAE would prefer the second system.
...
Typically, a trade off between these measures is expected.
