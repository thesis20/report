...
\end{equation}
To give an example of the calculation of Precision(L), Recall(L) and F1-measure, a top $k$ set and a relevant item set is defined to be used for the calculation
\begin{align}
    \text{Relevant-items} = \{item1, item3, item4, item5,\nonumber \\
     item6\} \nonumber \\
    \text{Top-items} = \{item1, item2, item3, item4\}\label{eqn:item-lists}
\end{align}
The intersection between the top-k set, L(u), and the relevant items, T(u), is a set containing three items $\{item1, item3, item4\}$.
The lengths of the L(u) and T(u) sets in this example are both four and five respectively.
These values are then plotted into the equations to get the precion and recall values.
...
\end{align*}
These values can then be used to calculate the F1-score.
\begin{align*}
...
The value of that is then divided by the minimum of either $N$ of the length of the relevant item set for the user, which in this case is $N$.
To find the MAP value, the AP values are simply summed together for each user in $U$ and divideed by $|U|$.
...
In the example, the gain $g_ui$ is set to be $1$ when a relevant item is being recommended, and $log_2$ is used.
...
Then the DCG* is calculated which is done by sorting the recommendation set so the most relevant items comes first, resulting in the biggest possible DCG value.
\begin{align*}
