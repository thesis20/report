...
For time units such as hours and smaller units, this can be done by discretizing it into categories such as \beginit{morning (6-11 am}, \textit{evening (11-2 pm)} and so on.
This allows the recommender to better group ratings together, since the movies a user watches at 1 pm may not be very different from 2 pm, but it may differ from morning time to night time.
