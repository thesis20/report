\subsection{LightGCN}
In \cite{LightGCN} they explored the NCGF method by exploring the effects that the nonlinear activation function and feature transformation had on the model's results.
They found that by removing these parts of the NGCF model they were able to achieve been results, meaning that they negatively affected the training of the model.
...
$N_u$ are the items that user $u$ have interacted with and $N_i$ are the users that have interacted with item $i$.
...
When the embeddings at the final layer has been computed, a layer combination is done to get a final presentation of the users and items.
...
Secondly, the embeddings at the different layers capture different features that would not be expressed if only the final layer's embeddings are used for the final embeddings.
