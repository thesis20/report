...
Typically, ratings in a set of ratings $R$ are divided into a training set $R_train$ used to learn $f$, and a test set $R_test$ used for evaluation prediction accuracy, which is usually done through MAE and RMSE.
...
Evaluating the quality of such method can be done by splitting $I$ into a training set $I_train$ used to learn $L$ and a test set $I_test$.
Let $T(u) \subset I_u \cap I_test$, where $I_u$ is the subset of items that have been rated by user $u$.

\todo{indsæt formler, mae, rmse, precision, recall}
\todo{ndcg}
\\\\
...
\begin{sidewaystable}[]
    ...
    \hline
       ...
       \cite{GameTheoretic}  & x                                &                                &                             & x                               &                        & \begin{tabular}[c]{@{}l@{}}RMSE, MAE, Recall@N, \\ DCG@N\end{tabular}                   & \begin{tabular}[c]{@{}l@{}}50\% training, 30\% testing,\\ 20\% validation\end{tabular}                     \\ \hline
       ...
       \cite{SinghUserItem}  & x                                &                                &                             &                                 &                        & MAE, Coverage                                                                           & 80\% training, 20\% test                                                                                                     \\ \hline
