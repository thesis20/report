...
This discretization could have been done differently, such as extracting seasonal data to look at summer and winter as context, or simply including less discretized categories, such as only the weekend and week for day of the week, or larger intervals for the time of the day.
...
For example, the smallest dataset used in the \textit{LightGCN}\cite{LightGCN} method has about a million interactions.
Therefore it could be interesting to run the experiment on a larger sized dataset to see if the graph-based methods achieve better performance.