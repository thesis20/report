...
Because of the trade-off, F1 is the most interesting metric for comparison, and the results for this metric can be found in \autoref{fig:comoda-f1}, \autoref{fig:yahoo-f1}, \autoref{fig:movielens-f1} and \autoref{fig:frappe-f1}.
...
As is evident from \autoref{fig:comoda-map}, \autoref{fig:yahoo-map}, \autoref{fig:movielens-map} and \autoref{fig:frappe-map} the MAP@N metric is very punishing, culminating in results of very small values approaching 0.
NDCG@N on the other hand, as shown on \autoref{fig:comoda-ndcg}, \autoref{fig:yahoo-ndcg}, \autoref{fig:movielens-ndcg} and \autoref{fig:frappe-ndcg}, tends to report more easily relatable results between 0 and 1 as an indicator.
...
None the less, the methods that are removed from their initial purpose have actually shown good results in other metrics, such as SVD performing the best in Precision@10 on the MovieLens dataset shown on \autoref{fig:movielens-precision}, even though it is meant for rating prediction.
...
As reported in \autoref{subsec:resultsofexperiment}, \textit{IS-UserBased} showed the highest Recall@10 score for the LDOS-CoMoDa and MovieLens datasets, shown on \autoref{fig:comoda-recall} and \autoref{fig:movielens-recall}, which are the datasets with contextual information.
...
To do a more complete comparison, the method was also run using side-information for item splitting for the Yahoo! dataset shown in \autoref{fig:yahoo-precision}, \autoref{fig:yahoo-recall} and \autoref{fig:yahoo-f1} where it once again had the best Recall@10 score, and the third lowest Precision@10.
...
The MAP@10 results are a slightly more inconclusive on the other hand, we see on \autoref{fig:yahoo-map} that the \textit{IS-UserBased} method achieves the best results on the Yahoo! dataset, while the \textit{CAMF} methods perform very poorly on the same dataset.
For the two other datasets, both methods give mediocre results, shown on \autoref{fig:comoda-map} and \autoref{fig:movielens-map}.
Finally, for the NDCG@10 metric shown on \autoref{fig:comoda-ndcg}, \autoref{fig:yahoo-ndcg}, \autoref{fig:movielens-ndcg} and \autoref{fig:frappe-ndcg}, we see that \textit{IS-Userbased} performs very well on the Yahoo! dataset, mediocre on LDOS-CoMoDa and poorly on MovieLens.
\\\\
