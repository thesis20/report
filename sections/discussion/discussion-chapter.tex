...
As reported in \autoref{subsec:resultsofexperiment}, \textit{IS-UserBased} showed the highest Recall@10 score for the LDOS-CoMoDa and MovieLens datasets, shown on \Cref{fig:comoda-recall,fig:movielens-recall}, which are the datasets with contextual information.
...
To do a more complete comparison, the method was also run using side-information for item splitting for the Yahoo! dataset shown in \Cref{fig:yahoo-precision,fig:yahoo-recall,fig:yahoo-f1} where it once again had the best Recall@10 score, and the third lowest Precision@10.
...
Finally, for the NDCG@10 metric shown on \Cref{fig:comoda-ndcg,fig:yahoo-ndcg,fig:movielens-ndcg,fig:frappe-ndcg}, we see that \textit{IS-Userbased} performs very well on the Yahoo! dataset, mediocre on LDOS-CoMoDa and poorly on MovieLens.
\\\\
